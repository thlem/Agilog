...
module.exports = function(agilogServer, passport){
    ...
    passport.authenticate('register-strategy', function(error, user, success){
      ...
      if(user){
        ...
        request.login(user, function(error){
          ...
          if(error){
            ...
            response.status(ResponseCodeConstant.INTERNAL_ERROR).json({message:ErrorMessageConstant.INTERNAL_ERROR+' : '+error});
          }
          // Sinon on retourne l'utilisateur au frontend
            ...
            console.log("[END][AuthenticationController][register]");
            response.status(ResponseCodeConstant.SUCCESS).json({message:success, user:user});
          }
        });
      }
      // Si l'enregistrement ne s'est pas effectué
        ...
        if(error){
          ...
          console.log("[END][AuthenticationController][register]");
          response.status(ResponseCodeConstant.BAD_REQUEST).json({message:error});
        }
        // Sinon on remonte une erreur générique
          ...
          console.log("[END][AuthenticationController][register]");
          response.status(ResponseCodeConstant.INTERNAL_ERROR).json({message:ErrorMessageConstant.INTERNAL_ERROR});
        }
      }
  ...
  agilogServer.get("/auth/logout", function(request, response){
    ...
    AuthenticationService.logout(request, function(){
      ...
      response.status(200).json({message:SuccessMessageConstant.SUCCESS_LOGOUT});
    });
  });
}
